...
provider "routeros" {
  ...
  username = var.username # env ROS_USERNAME or MIKROTIK_USER
  password = var.password # env ROS_PASSWORD or MIKROTIK_PASSWORD
...
provider "routeros" {
  alias    = "router1"
  hosturl  = var.hosturl1 # env ROS_HOSTURL or MIKROTIK_HOST
  username = var.username # env ROS_USERNAME or MIKROTIK_USER
  password = var.password # env ROS_PASSWORD or MIKROTIK_PASSWORD
  #ca_certificate = "/path/to/ca/certificate.pem" # env ROS_CA_CERTIFICATE or MIKROTIK_CA_CERTIFICATE
  insecure = true # env ROS_INSECURE or MIKROTIK_INSECURE
